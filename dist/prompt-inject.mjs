...
function selectForInjection(results, { threshold = 1, topK = 3, exclude = /* @__PURE__ */ new Set() } = {}) {
  return results.filter((r) => r.score >= threshold && !exclude.has(r.displayName)).slice(0, topK);
}

// src/inject.mjs
var PRESETS = {
  off: { topK: 0, threshold: Infinity },
  minimal: { topK: 1, threshold: 150 },
  // only the single strongest match
  medium: { topK: 3, threshold: 100 }
  // up to 3 strong matches (default)
};
function scaleThreshold(base, corpusSize, ref = 1700) {
  if (!corpusSize || corpusSize >= ref) return base;
  return Math.max(10, Math.round(base * (corpusSize / ref)));
}
function buildInjection({ prompt, skills, seen = /* @__PURE__ */ new Set(), preset = "medium", searcher = null, threshold, topK } = {}) {
  const cfg = PRESETS[preset] || PRESETS.medium;
  const effTopK = topK ?? cfg.topK;
  const corpusSize = Array.isArray(skills) ? skills.length : void 0;
  ...
  const text = `[skillseek] Possibly relevant installed skills for this task (consider invoking via Skill, or call skill_search):
${lines}`;
  return { text, picked };
}
...
function stateFile(sessionId) {
  const dir = path2.join(os2.homedir(), ".claude", ".skillseek-state");
  fs2.mkdirSync(dir, { recursive: true });
  return path2.join(dir, `${(sessionId || "default").replace(/[^a-zA-Z0-9_-]/g, "_")}.json`);
}
try {
